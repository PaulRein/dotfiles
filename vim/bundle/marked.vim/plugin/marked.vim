...
function s:OpenMarked(background)
  ...
  silent exe "!open -a \"Marked 2\" ".(a:background ? '-g' : '')." '".l:filename."'"
...
augroup marked_commands
  autocmd!
  autocmd FileType markdown,mkd,mmd command! -buffer -bang MarkedOpen :call s:OpenMarked(<bang>0)
  autocmd FileType markdown,mkd,mmd command! -buffer MarkedQuit :call s:QuitMarked(expand('%:p'))
augroup END
